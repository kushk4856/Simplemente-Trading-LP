    ...
    if (number_field.validity.patternMismatch) {
      if (number_field.value.startsWith("9")) {
        number_field.setCustomValidity(
          "It seems the number is invalid. Mobile numbers must start with 9 and be exactly 8 digits long."
        );
      } else if (number_field.value.startsWith("2")) {
        number_field.setCustomValidity(
          "It seems the number is invalid. Landline numbers must start with 2 and be exactly 8 digits long."
        );
      } else {
        number_field.setCustomValidity(
          "It seems the number is invalid. The number must start with either 9 or 2 and be exactly 8 digits long."
        );
      }
    } else {
      number_field.setCustomValidity("");
